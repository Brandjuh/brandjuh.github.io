...
const App: React.FC = () => {
  ...
  return (
    <div className={styles.overview.container}>
      {/* Header met titel, voortgang en dropdown */}
      <div className={styles.overview.header}>
        <h2 className={styles.overview.title}>{strings[locale].progressTitle}</h2>
        <div className="flex items-center space-x-4">
          {/* Voortgang */}
          <div className={styles.overview.stats}>
            <div className={styles.overview.statsCount}>
              {handledCount}/{vops.length}
            </div>
            <div className={styles.overview.statsLabel}>
              {pendingCount} {strings[locale].remaining}
            </div>
          </div>
          {/* Taalkeuze */}
          <select
            ...
            className="ml-4 p-1 rounded bg-gray-800 text-white"
          >
          ...
          <button
            ...
            className="p-2 rounded-full bg-gray-700 text-gray-100"
          >
            {darkMode ? '☀️' : '🌙'}
          </button>
        </div>
      ...
      <div className={styles.quickActions}>
        ...
        <button
          onClick={() => vops.forEach(v => !v.done && toggleDone(v.id))}
          className="px-4 py-2 bg-green-600 rounded hover:bg-green-700"
        >
          {strings[locale].markAll}
        </button>
        <button
          onClick={() => vops.forEach(v => !v.hidden && toggleHidden(v.id))}
